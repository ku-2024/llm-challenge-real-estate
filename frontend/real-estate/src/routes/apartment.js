...
import { car, house } from "../icon";

const Apartment = () => {
  const icons = [car, house];
  ...
  return (
    <div className="bg-slate-100">
      ...
      <div class="ml-[18vw] px-[2%] mx-[2vw]">
        ...
        <DetailInfo id="A-env" title="환경(단지,조경)" info={info} icon={car} />
        <DetailInfo
          id="A-community"
          title="커뮤니티"
          info={info}
          icon={house}
        />
        <DetailInfo id="A-attribute" title="동별 특징" info={info} icon={car} />
        <DetailInfo id="A-infra" title="주변 상권" info={info} icon={house} />
        <DetailInfo id="A-traffic" title="교통" info={info} icon={car} />
        <DetailInfo id="A-school" title="학군" info={info} icon={house} />
        <DetailInfo id="A-sound" title="소음" info={info} icon={car} />
        <DetailInfo id="A-parking" title="주차" info={info} icon={house} />
      </div>
    </div>
  );
};
